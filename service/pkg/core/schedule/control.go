...
func (i *control) Connect(ctx context.Context) {
	...
	i.wsClient.HandleRequestWithKeys(ginCtx.Writer, ginCtx.Request, map[string]any{
		LABINFO: labInfo,
		"ctx":   ctx,
		"uuid":  lab.UUID,
	})
...
func (i *control) stopWorkflowTask(ctx context.Context, info *engine.WorkflowInfo) error {
	...
	if err := task.Task.Stop(ctx); err != nil {
...
func (i *control) initWebSocket(_ context.Context) {
	i.wsClient.HandleClose(func(s *melody.Session, _ int, s2 string) error {
		// 关闭之后的回调
		ctx, _ := s.Get("ctx")
		...
		i.consumer.RemoveUser(gCtx, uuid.String())
		return nil
	})
	...
	i.wsClient.HandleSentMessage(func(s *melody.Session, b []byte) {
		// 发送完字符串消息后的回调
	})

	i.wsClient.HandleSentMessageBinary(func(s *melody.Session, b []byte) {
		// 发送完二进制消息后的回调
	})
}
